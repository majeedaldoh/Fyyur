...
def show_venue(venue_id):
  ...
  if not venue_list:
    ...
    if num_upcoming_shows == 0 and num_past_shows == 0:
      data = ({
        'id' : venue_list.id,
    ...
    elif num_upcoming_shows > 0 and num_past_shows == 0:
      data = ({
        'id' : venue_list.id,
        'name': venue_list.name,
        'address': venue_list.address,
  ...
  if form.validate():
    ...
    try:
      new_venue = Venue(
        ...
        genres=form.getlistgenres.data,
...
def show_artist(artist_id):
  ...
  if not artist_list:
    ...
    upcoming_shows = db.session.query(Show).join(Venue).filter(
      ...
      Show.start_time >= datetime.datetime.now().now()
    )
